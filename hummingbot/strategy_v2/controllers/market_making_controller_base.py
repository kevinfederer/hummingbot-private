...
class MarketMakingControllerConfigBase(ControllerConfigBase):
    ...
    capital_allocation_stages: int = Field(
        default=1, ge=1,
        json_schema_extra={
            "prompt": "Enter the number of capital allocation stages (e.g., 3 uses 1/3 of total each cycle): ",
            "prompt_on_new": True, "is_updatable": True}
    )
    ...
    executor_refresh_time_jitter: float = Field(
        default=0,
        json_schema_extra={
            "prompt": "Enter a jitter (in seconds) added to refresh time (e.g., 1 for random 0-1s): ",
            "prompt_on_new": True, "is_updatable": True}
    )
    ...
    cooldown_time_jitter: float = Field(
        default=0,
        json_schema_extra={
            "prompt": "Enter a jitter (in seconds) added to cooldown time (e.g., 1 for random 0-1s): ",
            "prompt_on_new": True, "is_updatable": True}
    )
    leverage: int = Field(
        default=20,
    ...
    def get_spreads_and_amounts_in_quote(self, trade_type: TradeType) -> Tuple[List[float], List[float]]:
        ...
        # Use only a fraction of total capital per cycle
        effective_total_quote = self.total_amount_quote / Decimal(self.capital_allocation_stages)
        ...
        return spreads, [amt_pct * effective_total_quote for amt_pct in normalized_amounts_pct]
...
class MarketMakingControllerBase(ControllerBase):
    ...
    def get_levels_to_execute(self) -> List[str]:
        ...
        if self.config.cooldown_time_jitter > 0:
            cooldown_time += random.random() * self.config.cooldown_time_jitter
        working_levels = self.filter_executors(
            executors=self.executors_info,
            filter_func=lambda x: x.is_active or (
                x.close_type == CloseType.STOP_LOSS and
                self.market_data_provider.time() - x.close_timestamp < cooldown_time
            )
        )
    ...
    def executors_to_refresh(self) -> List[ExecutorAction]:
        executors_to_refresh = self.filter_executors(
            ...
            self.market_data_provider.time() - x.timestamp >
            (self.config.executor_refresh_time +
             (random.random() * self.config.executor_refresh_time_jitter
              if self.config.executor_refresh_time_jitter > 0 else 0))
        )
